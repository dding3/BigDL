...
//                       localStates: Array[Table],
  ...
  private[optim] def optimize[T: ClassTag](
    ...
    println("paritionNum: " + partitionNum)
    ...
    while (!endWhen(driverState)) {
      ...
      metrics.set("get gradient blocks", 0.0, sc, Engine.nodeNumber().get)
      metrics.set("tell master gradient", 0.0, sc, partitionNum)      
...
//          println(s"finishedthreads size: ${finishedThreads.size} size: ${tensorBuffer.head._2.size(1)}")
          ...
          val t = parameters.sendGradientPartition(cached.gradient, TaskContext.getPartitionId())
          ...
          time = System.nanoTime()
          parameters.test(t)
          driverMetrics.add("tell master gradient", System.nanoTime() - time)
      ...
      dataRDD.mapPartitions { iter =>
        ...
        parameters.synchronized {
          if (!parameters.job2Start) {
            var t = System.nanoTime()
            val blockIds = parameters.test2()
            driverMetrics.add("get gradient blocks", System.nanoTime() - t)
            
            t = System.nanoTime()
            val gradient = parameters.aggregateLocalGradient(blockIds)
            parameters.putGradients(gradient)
            parameters.job2Start = true
...
//          modelCache.localStates.head("epoch") = driverState[Int]("epoch")
          ...
          ParameterManager2.synchronized {
            if (!parameters.job3Start) {
              ...
              parameters.job3Start = true
              parameters.job2Start = false
              
              var time = System.nanoTime()
              val gradients = parameters.readGradientExecutor[T]()
              gradients.div(ev.fromType(finishedModelNum))
              
//              println("gradients: " + gradients)
              val weights = parameters.readWeightExecutor[T]()
...
//          val parameters = ParameterManager2.get
//          parameters.synchronized {
//            if (!parameters.job4Start) {
//              val t = System.nanoTime()
//              parameters.sendWeightExecutor()
//              parameters.job4Start = true
//              parameters.job3Start = false
//              driverMetrics.add("send weights average", System.nanoTime() - t)
//            }
//          }
//          Iterator.empty
//        }.count()
...
//            partitionNum, parameterSize, false)
//        }
//        Iterator.empty
//      }
//    }.count()
...
//        (localModel, weights, grads, localCriterion, localState)
      ...
      val weights = cached.head._2
      ParameterManager2.synchronized {
        var parameter = ParameterManager2.get()
        if (parameter == null) {
          val executorId = SparkEnv.get.executorId
