...
//    val offset = partitionId * taskSize + math.min(partitionId, extraSize)
//    val length = taskSize + (if (partitionId < extraSize) 1 else 0)
//    val blockId = pm.getGradientPartitionId()
//    val gradient = BlockManagerWrapper.getLocal(blockId).map(_.data.next()) match {
//      case Some(x) =>
//        x.asInstanceOf[Tensor[T]]
//
//      case None =>
//        throw new Exception("Not find gradient in ParameterManager")
//    }
//    gradient.narrow(1, offset + 1, length)
//  }
//
...
////      localParameter.set(BlockManagerWrapper.getLocal(pm.getWeightId())
...
////    }
//  
//  //  def aggregrateGradientParition1(params: Array[CompressedTensor[T]]): Unit = {
////    val bm = SparkEnv.get.blockManager
...
//
//  def putGradients(parameter: Tensor[T]): Unit = {
//    val gradientPartitionId = getGradientPartitionId()
//    BlockManagerWrapper.removeBlock(gradientPartitionId)
//    BlockManagerWrapper.putSingle(gradientPartitionId,
//      parameter, StorageLevel.MEMORY_AND_DISK, tellMaster = false)
//    pm.blockIds.synchronized(pm.blockIds.append(gradientPartitionId))
//  }
//  
...
//    pm.blockIds.synchronized(pm.blockIds.append(weightsId))
